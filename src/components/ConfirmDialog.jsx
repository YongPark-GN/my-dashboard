  ...
  return createPortal(
    <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', zIndex: 9999999, backgroundColor: 'rgba(0,0,0,0.55)', backdropFilter: 'blur(12px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: 'var(--editor-bg)', border: '1px solid var(--glass-border)', borderRadius: '24px', padding: '32px', width: '320px', color: 'var(--txt)', boxShadow: '0 20px 40px rgba(0,0,0,0.4)', textAlign: 'center' }}>
        <p style={{ margin: '0 0 24px 0', fontSize: '1rem', color: 'var(--txt)', lineHeight: '1.5', whiteSpace: 'pre-wrap' }}>{message}</p>
        <div style={{ display: 'flex', gap: '12px' }}>
          <button onClick={onCancel} style={{ flex: 1, background: 'var(--chip-strong)', border: 'none', color: 'var(--txt)', padding: '12px', borderRadius: '12px', cursor: 'pointer', fontSize: '0.95rem', fontWeight: '600' }}>취소</button>
          <button onClick={onConfirm} style={{ flex: 1, background: 'var(--danger)', border: 'none', color: '#fff', padding: '12px', borderRadius: '12px', cursor: 'pointer', fontSize: '0.95rem', fontWeight: '700' }}>{confirmLabel}</button>
        </div>
      </div>
    </div>,
    document.body
